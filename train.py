...
import mindtorch.torch.optim.lr_scheduler as scheduler
...
def train(rundir,source_temp,target_temp,source_data_path,source_train_set,source_test_set,target_data_path,target_train_set,target_test_set,models, criterion, optimizers, batch_size, epochs,eval_interval, lamb1,lamb2,lamb3,seed=0, device_type=('cuda:0' if torch.cuda.is_available() else 'cpu'), ifsave=True, load_model=False, model_path='/models/best.pt'):
  ...
  def forward_fna(source_data, source_label):
    ...
    return loss_stepA, predicts_p1, predicts_p2

# 反向梯度定义
  grad_fna = ms.value_and_grad(forward_fna, None, optimizers['parameters'].parameters, has_aux=True)

# 单步训练定义
  def train_stepa(source_data, source_label):

    (loss_stepA, _, _), grads = grad_fna(source_data, source_label)

    optimizers['parameters'](grads)
    return loss_stepA

  
  def forward_fnb(source_data, source_label):
      source_features = models['lstm'](models['conv'](source_data))
      predicts_p1 = models['fc1'](source_features).squeeze()
      predicts_p2 = models['fc2'](source_features).squeeze()
      loss_stepB = lamb1*(criterion(predicts_p1,source_label) + criterion(predicts_p2,source_label) - discrepancy(predicts_p1, predicts_p2))
      return loss_stepB, predicts_p1, predicts_p2

  grad_fnb = ms.ops.value_and_grad(forward_fnb, None, optimizers['parameters'].parameters, has_aux=True)

  def train_stepb(source_data, source_label):
      (loss_stepB, _, _), grads = grad_fnb(source_data, source_label)
      optimizers['parameters'](grads)
      return loss_stepB


  def forward_fnc(target_data, target_label):
    ...
    return loss_target,loss_stepC, predicts_p1, predicts_p2

  grad_fnc = ms.ops.value_and_grad(forward_fnc, None, optimizers['parameters'].parameters, has_aux=True)

  def train_stepc(target_data, target_label):

      (loss_target,loss_stepC, _, _), grads = grad_fnc(target_data, target_label)
      optimizers['parameters'](grads)
      return loss_target,loss_stepC
  ...
  for temp_idx in range(1):
    ...
    for epoch in range(epochs):
      ...
      for i, ((source_data, source_label), (target_data, target_label)) in enumerate(tqdm_mix):
        ...
        loss_stepA=train_stepa(source_data, source_label)
        ...
        loss_stepB=train_stepb(source_data, source_label)
        ...
        # optimizers['conv'].step()
        # optimizers['lstm'].step()
        loss_target,loss_stepC=train_stepc(target_data, target_label)
