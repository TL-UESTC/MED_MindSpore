...
LG_train_set = []
LG_test_set = []
INR_train_set = ['degC_BJDST_50SOC']
INR_test_set = ['degC_BJDST_80SOC']
  ...
  def __init__(self, data_path,temp, set, mode='train'):
    ...
    if mode == 'train':
      ...
      for i in range(len(set)):
        ...
        #print('shape:{},{}'.format(self.X[i].shape,self.Y[i].shape))
        self.X[i] = torch.from_numpy(self.X[i].astype(np.float32))
        self.Y[i] = torch.from_numpy(self.Y[i].astype(np.float32))
